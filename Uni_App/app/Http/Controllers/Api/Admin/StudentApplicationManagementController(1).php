...
class StudentApplicationManagementController extends Controller
{
        ...
        if ($request->filled('search')) {
            ...
            $query->where(function($q) use ($searchTerm) {
                $q->where('full_name', 'like', "%{$searchTerm}%")
                  ->orWhere('national_id_number', 'like', "%{$searchTerm}%")
    ...
    public function approve(int $id): JsonResponse
    {
        ...
        try {
            ...
            try {
                Mail::to($app->email_address)->send(new ApplicationApproved($app, $studentNumber, $tempPassword));
            } catch (\Exception $e) {
    ...
    public function reject(Request $request, int $id): JsonResponse
    {
        ...
        if ($app->application_status === 'completed') {
            return response()->json(['success' => false, 'message' => 'لا يمكن رفض طلب تم قبوله مسبقاً'], 422);
        }
        ...
        try {
            Mail::to($app->email_address)->send(new ApplicationRejected($app, $request->input('rejection_reason')));
        } catch (\Exception $e) {
